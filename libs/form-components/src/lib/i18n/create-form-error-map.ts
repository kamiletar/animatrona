...
function extractParams(issue: ZodIssue): TranslateParams {
  ...
  // too_small / too_big
  if (issue.minimum !== undefined) params.minimum = issue.minimum
  if (issue.maximum !== undefined) params.maximum = issue.maximum
  if (issue.inclusive !== undefined) params.inclusive = issue.inclusive

  // invalid_type
  if (issue.expected !== undefined) params.expected = issue.expected
