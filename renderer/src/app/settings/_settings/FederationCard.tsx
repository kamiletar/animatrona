    ...
    <Box>
      <HStack mb={4} gap={3} justify="space-between">
        ...
        {federationEnabled && trackers.list.length > 0 && (
          <Button size="xs" variant="ghost" onClick={() => void onSyncAll()} loading={sync.isSyncing}>
                  ...
                  <HStack gap={1}>
                    {isBlocked ? (
                      ...
                      <>
                        ...
                        <Button size="xs" variant="ghost" colorPalette="red" onClick={() => void onBlock(tracker.id)}>
                          <Icon as={LuBan} mr={1} />
                          Блок
                        </Button>
                      </>
...
function SyncResultsSection({ sync }: { sync: ReturnType<typeof useFederation>['sync'] }) {
  if (sync.lastResults.length === 0) return null
