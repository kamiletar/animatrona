...
export default function SubscriptionLibraryPage({ params }: SubscriptionPageProps) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (!subResult.success || !subResult.data) {
        ...
        navigateTo('/settings/p2p-sharing')
        return
      }
  ...
  const handleImport = useCallback(async (manifestCid: string) => {
    const api = window.electronAPI?.animeManifest
    if (!api) return

    setImportingCid(manifestCid)
    try {
      const result = await api.import(manifestCid)
      if (result.success && result.data) {
        toaster.success({
          title: 'Аниме импортировано',
          description: result.data.animeName,
        })
        navigateTo(`/library/${result.data.animeId}`)
      } else {
    ...
    <Box>
      ...
      <Box p={6}>
        ...
        <HStack mb={4} justify="space-between">
          <HStack gap={2}>
            <Button size="sm" variant="ghost" onClick={() => navigateTo('/settings/p2p-sharing')}>
              <Icon as={LuArrowLeft} />
              Назад
            </Button>
        ...
        {!library ? (
          <VStack py={8} gap={4}>
            <Text color="fg.subtle">
              Нет данных — нажмите «Обновить» для загрузки библиотеки через IPNS
            </Text>
            <Button
              colorPalette="purple"
              onClick={() => void handleRefresh()}
          ...
          >
            {library.animes.map((anime, idx) => (
              <PublishedAnimeCard
                ...
                isImporting={importingCid === anime.manifestCid}
              />
            ))}
          </Grid>
        )}
...
interface PublishedAnimeCardProps {
  ...
  onImport: (manifestCid: string) => Promise<void>
  isImporting: boolean
}
      ...
      <VStack align="start" p={3} gap={1}>
        ...
        {anime.manifestCid && (
          <Button
            ...
            w="full"
            mt={1}
            onClick={() => void onImport(anime.manifestCid!)}
            loading={isImporting}
          >
            <Icon as={LuDownload} />
            Импортировать
