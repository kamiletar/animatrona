 ...
 * - http://localhost:{port}/ipfs/ для IPFS контента через gateway
 *
 * Реальный порт gateway устанавливается через setGatewayBaseUrl()
 * при старте приложения (см. AppShell). Порт 8765 — только fallback
 * на случай если gateway уже запущен до инициализации renderer.
 */

/** Базовый URL IPFS gateway (обновляется при старте/остановке gateway) */
let _gatewayBaseUrl = 'http://localhost:8765'

/**
 * Установить реальный базовый URL gateway.
 * Вызывается из AppShell при старте приложения и при смене состояния gateway.
 */
export function setGatewayBaseUrl(baseUrl: string | null): void {
  _gatewayBaseUrl = baseUrl ?? 'http://localhost:8765'
}
