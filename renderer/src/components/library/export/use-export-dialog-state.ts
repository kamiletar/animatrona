...
export function useExportDialogState({ anime, defaultExportPath, open, onOpenChange }: UseExportDialogStateProps) {
  ...
  const [progress, _setProgress] = useState<SeriesExportProgress | null>(null)
  const [result, _setResult] = useState<ExportResult | null>(null)
      ...
      if (result.success) {
        ...
        if (webResourceMode === 'publish' && result.rootCid) {
          ...
          setPublishResult({
            ...
            gatewayUrl: result.gatewayUrl || `http://localhost:8765/ipfs/${result.rootCid}`,
