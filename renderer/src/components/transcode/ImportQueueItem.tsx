...
export const ImportQueueItem = memo(
  function ImportQueueItem({ item, isCurrent, onRemove, onRetry, onEdit, isFocused, onFocus }: ImportQueueItemProps) {
  ...
  (prev, next) => {
    ...
    if (prev.item.forceCpu !== next.item.forceCpu) return false

    // Счётчик завершённых видео — для обновления "3/5 видео"
    if (prev.item.detailProgress?.videoCompleted !== next.item.detailProgress?.videoCompleted) return false

    // currentFileName — для обновления текста под прогресс-баром
    if (prev.item.currentFileName !== next.item.currentFileName) return false
