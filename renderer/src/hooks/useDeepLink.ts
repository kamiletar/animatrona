...
export interface DeepLinkAction {
  type: 'party_join' | 'friend_add' | 'unknown'
...
export function useDeepLink(): UseDeepLinkReturn {
  ...
  const generateWatchPartyInvite = useCallback(
    async (
      ...
      if (!window.electronAPI?.deepLink) return null

      try {
        const result = await window.electronAPI.deepLink.generateWatchPartyInvite(
          roomId,
          roomName,
          hostName,
          animeName,
  ...
  const showInviteNotification = useCallback(async (invite: WatchPartyInvite): Promise<boolean> => {
    if (!window.electronAPI?.deepLink) return false
  ...
  useEffect(() => {
    if (!window.electronAPI?.deepLink) return
