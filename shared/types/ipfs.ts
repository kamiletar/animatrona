...
/**
 * @deprecated Используй IpfsServiceStatus
 */
export type HeliaServiceStatus = IpfsServiceStatus
...
/**
 * Результат IPC операции IPFS
 */
export interface IpfsOperationResult<T = void> {
  success: boolean
  data?: T
  error?: string
}
...
/**
 * Опции для добавления файла
 */
export interface IpfsAddOptions {
  /** Оборачивать файл в директорию */
  wrapWithDirectory?: boolean
  /** Прогресс-callback (bytes added) */
  onProgress?: (bytes: number) => void
}

/**
 * Опции для добавления директории
 */
export interface IpfsAddDirectoryOptions extends IpfsAddOptions {
  /** Рекурсивно добавлять поддиректории */
  recursive?: boolean
  /** Паттерны для игнорирования (glob) */
  ignore?: string[]
}
...
/**
 * Прогресс добавления файла
 */
export interface IpfsAddProgress {
  /** Имя файла */
  fileName: string
  /** Добавлено байт */
  bytesAdded: number
 ...
 *
 * minConnections: 20 — как IPFS Desktop (Kubo default: 100)
 * maxConnections: 100 — больше для лучшей connectivity
 *
 * Раньше было 5/50, что слишком мало для активного DHT discovery.
 */
...
/**
 * @deprecated Используй IpfsServiceEvent
 */
export type HeliaServiceEvent = IpfsServiceEvent
