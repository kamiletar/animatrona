...
export function PlayerPage() {
  ...
  const nextEpisode = useMemo(() => {
    ...
    if (currentIndex === -1 || currentIndex >= anime.episodes.length - 1) return null
  ...
  const currentAudioTrack = useMemo((): AudioTrack | null => {
    if (!episode || !selectedAudioTrack) return null
  ...
  const currentSubtitleTrack = useMemo((): SubtitleTrack | null => {
    if (!episode || !selectedSubtitleTrack) return null
  ...
  const isAssSubtitle = useMemo(() => {
    if (!currentSubtitleTrack) return false
        ...
        if (!ep) {
          ...
          if (animePref) {
            ...
            if (animePref.subtitleTrackId !== undefined) {
              ...
              if (hasTrack) setSelectedSubtitleTrack(animePref.subtitleTrackId)
            }
          }
  ...
  useEffect(() => {
    if (!showControls || showTrackMenu || showNextEpisode || showSpeedMenu) return
  ...
  const handleVideoAreaTap = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (showTrackMenu || showNextEpisode || showSpeedMenu || showPlayOverlay) return
