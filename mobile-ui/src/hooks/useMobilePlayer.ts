...
export function useMobilePlayer(options: UseMobilePlayerOptions): UseMobilePlayerReturn {
  ...
  useEffect(() => {
    ...
    const handleLoadedMetadata = () => {
      if (!isMounted) return
      console.log('[MobilePlayer] loadedmetadata:', {
    ...
    const handleLoadedData = () => {
      if (!isMounted) return
      console.log('[MobilePlayer] loadeddata — first frame available')
    ...
    const handleWaiting = () => {
      if (!isMounted) return
      console.log('[MobilePlayer] waiting — buffering')
      setIsLoading(true)
    }

    const handleCanPlay = () => {
      if (!isMounted) return
      console.log('[MobilePlayer] canplay')
      setIsLoading(false)
    }

    const handleCanPlayThrough = () => {
      if (!isMounted) return
      console.log('[MobilePlayer] canplaythrough — enough data to play through')
      if (loadTimeoutId) {
        clearTimeout(loadTimeoutId)
        loadTimeoutId = null
      }
    ...
    const handleSuspend = () => {
      if (!isMounted) return
      console.log('[MobilePlayer] suspend — loading intentionally halted')
    }

    const handleProgress = () => {
      if (!isMounted) return
      // Логируем прогресс загрузки только каждые 10 вызовов
      if (video.buffered.length > 0) {
        const buffered = video.buffered.end(video.buffered.length - 1)
        if (buffered > 0 && Math.floor(buffered) % 10 === 0) {
          console.log('[MobilePlayer] progress — buffered:', buffered.toFixed(1), 's')
        }
      }
    }
    ...
    // Загружаем видео
    console.log('[MobilePlayer] Loading video:', videoUrl)
    video.src = videoUrl
  ...
  // Синхронизация аудио
  useEffect(() => {
    if (!isReady || !usesSeparateAudio) return
