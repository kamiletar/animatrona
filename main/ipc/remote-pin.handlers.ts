...
export function registerRemotePinHandlers(): void {
  ...
  createHandler('remotePin:getConfig', () => makeSafeConfig(loadRemotePinConfig()))

  // Обновить конфигурацию
  createHandler('remotePin:updateConfig', (updates: Partial<RemotePinConfig>) => {
    const config = updateRemotePinConfig(updates)
  ...
  createHandler('remotePin:updatePinataConfig', (updates: Partial<PinataConfig>) => {
    const config = updatePinataConfig(updates)
    const safeConfig = makeSafeConfig(config)
    broadcastToWindows('remotePin:configUpdated', safeConfig)
    return safeConfig
  })
