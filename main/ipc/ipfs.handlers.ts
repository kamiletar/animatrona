 ...
 * - ipfs:gateway:start — Запустить HTTP Gateway
 * - ipfs:gateway:stop — Остановить HTTP Gateway
 * - ipfs:gateway:status — Статус Gateway
 * - ipfs:gateway:getUrl — Получить URL для CID
...
export function registerIpfsHandlers(): void {
  ...
  // === HTTP Gateway ===
  createHandler('ipfs:gateway:start', (options?: { port?: number }) => gatewayServer.start(options))
  createHandler('ipfs:gateway:stop', () => gatewayServer.stop())
  createHandler('ipfs:gateway:status', () => gatewayServer.getStatus())
  createHandler('ipfs:gateway:getUrl', (cid: string, path?: string) => gatewayServer.getContentUrl(cid, path))
  ...
  forwardEvents(gatewayServer, 'ipfs:gateway', {
    started: 'started',
    stopped: 'stopped',
  })
