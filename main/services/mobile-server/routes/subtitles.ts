...
function convertAssToVtt(ass: string): string {
  ...
  if (startIdx === -1 || endIdx === -1 || textIdx === -1) return lines.join('\n')
...
export async function handleIpfsSubtitlesRequest(
  ...
  const gatewayUrl = `http://localhost:${GATEWAY_PORT}/ipfs/${cid}`
