...
export async function handleLastWatchedRequest(res: ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (!lastProgress) {
      ...
      const animeWithProgress = await db.anime.findFirst({
        ...
        select: {
          ...
          episodes: {
            where: {
              OR: [{ transcodedCid: { not: null } }, { videoPath: { not: null } }],
            },
            orderBy: { number: 'asc' },
            take: 1,
            select: {
      ...
      res.end(
        JSON.stringify({
          anime: {
            ...
            posterPath: animeWithProgress.poster?.path ?? null,
          },
          episode: {
            id: episode.id,
    ...
    if (!lastProgress.episode.transcodedCid && !lastProgress.episode.videoPath) {
      res.writeHead(404, { 'Content-Type': 'application/json' })
    ...
    const response = {
      anime: {
        ...
        posterPath: lastProgress.anime.poster?.path ?? null,
      },
      episode: {
