...
const CONFIG_FILE = 'remote-pinning.json'
 ...
 */
export function loadRemotePinConfig(): RemotePinConfig {
  try {
    const filePath = getConfigPath()
    if (!fs.existsSync(filePath)) {
      return { ...DEFAULT_REMOTE_PIN_CONFIG }
    }
    const data = fs.readFileSync(filePath, 'utf-8')
    return { ...DEFAULT_REMOTE_PIN_CONFIG, ...JSON.parse(data) }
  } catch (error) {
    log.error('Ошибка загрузки конфигурации', { error })
    return { ...DEFAULT_REMOTE_PIN_CONFIG }
  }
}
 ...
 */
export function saveRemotePinConfig(config: RemotePinConfig): void {
  try {
    const filePath = getConfigPath()
    fs.writeFileSync(filePath, JSON.stringify(config, null, 2), 'utf-8')
  } catch (error) {
    log.error('Ошибка сохранения конфигурации', { error })
    throw error
  }
}
 ...
 */
export function updateRemotePinConfig(updates: Partial<RemotePinConfig>): RemotePinConfig {
  const config = loadRemotePinConfig()
  const updated = { ...config, ...updates }
  saveRemotePinConfig(updated)
  return updated
}
 ...
 */
export function updatePinataConfig(updates: Partial<PinataConfig>): RemotePinConfig {
  const config = loadRemotePinConfig()
  config.pinata = { ...config.pinata, ...updates }
  saveRemotePinConfig(config)
  return config
}
