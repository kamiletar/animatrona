...
export async function addFile(filePath: string, onProgress?: (bytes: number) => void): Promise<IpfsAddResult> {
  log.info('addFile → UnifiedIPFS', { filePath })
  return UnifiedIPFS.addFile(filePath, onProgress)
}
...
export async function addBytes(content: Buffer): Promise<string> {
  log.info('addBytes → UnifiedIPFS', { bytes: content.length })
  return UnifiedIPFS.addBytes(content)
}
