...
export function updateSubscription(
  ...
  // Обновляем поля
  if (data.displayName !== undefined) subscription.displayName = data.displayName
  if (data.autoPin !== undefined) subscription.autoPin = data.autoPin
  if (data.autoPinLimit !== undefined) subscription.autoPinLimit = data.autoPinLimit
  if (data.lastKnownCid !== undefined) subscription.lastKnownCid = data.lastKnownCid
  if (data.lastCheckedAt !== undefined) subscription.lastCheckedAt = data.lastCheckedAt
...
export async function refreshSubscription(id: string): Promise<SubscriptionRefreshResult> {
  ...
  try {
    ...
    const content = await cat(newCid)
