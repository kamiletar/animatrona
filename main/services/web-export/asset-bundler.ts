...
async function downloadFromIpfs(cid: string, destPath: string): Promise<void> {
  const url = `http://localhost:${IPFS_GATEWAY_PORT}/ipfs/${cid}`
...
export async function buildDirectoryStructure(
  ...
  for (const ep of episodes) {
    ...
    for (const track of ep.subtitleTracks) {
      const key = `${track.language}:${track.title || 'default'}`
      if (!config.selectedSubtitleKeys.includes(key)) continue
...
<body>
  ...
  <script>
(function(){
  var GATEWAYS = ['http://localhost:8765/ipfs/', 'https://ipfs.io/ipfs/'];
