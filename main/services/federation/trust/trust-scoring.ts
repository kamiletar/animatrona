...
export function getTrustLevel(score: number, isBlocked = false): TrustLevel {
  if (isBlocked) return 0 // BLOCKED

  if (score >= TRUST_THRESHOLDS.VERIFIED) return 4 // VERIFIED
  if (score >= TRUST_THRESHOLDS.TRUSTED) return 3 // TRUSTED
  if (score >= TRUST_THRESHOLDS.KNOWN) return 2 // KNOWN
  return 1 // UNTRUSTED
}
