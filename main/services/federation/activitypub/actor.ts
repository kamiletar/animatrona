...
export async function createLocalActor(options: CreateActorOptions): Promise<ActivityPubActor> {
  ...
  // Вычисляем общий размер (сумма transcodedSize)
  const sizeResult = await prisma.episode.aggregate({
    _sum: { transcodedSize: true },
  })
  const totalSize = Number(sizeResult._sum.transcodedSize ?? 0)
...
export function parseRemoteActor(json: unknown): ActivityPubActor | null {
  ...
  if (obj.publicKey && typeof obj.publicKey === 'object') {
    ...
    if (
      typeof pk.id === 'string'
      && typeof pk.owner === 'string'
      && typeof pk.publicKeyPem === 'string'
    ) {
      publicKey = {
        id: pk.id,
        owner: pk.owner,
